...
func init() {
	plugin.RegisterProvider(&GitLabRepository{})
}
...
func (repo *GitLabRepository) GetInfo() (*plugin.RepositoryInfo, error) {
	project, _, err := repo.client.Projects.GetProject(repo.projectID, nil)

	if err != nil {
		return nil, err
	}
	return &plugin.RepositoryInfo{
		Owner:         "",
		Repo:          "",
...
func (repo *GitLabRepository) CreateRelease(release *plugin.CreateReleaseConfig) error {
